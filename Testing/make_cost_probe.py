# Builds an offscreen cost harness: the app's sources, minus the bootstrap, plus a probe
# that drives the real view and times every repaint it asks for.
import io, os, shutil

root = os.environ.get("TESTING_DIR", ".build/testing")
pkg = root + "/cost"
src = pkg + "/Sources/COST"
shutil.rmtree(pkg, ignore_errors=True)
os.makedirs(src)
io.open(pkg + "/Package.swift", "w", encoding="utf-8").write(
    io.open("Package.swift", encoding="utf-8").read().replace("ScreenDrawOverlay", "COST"))
...
shutil.copy("Testing/probes/cost.swift", src + "/main.swift")
print("cost harness built")
